...
class aug_check(QWidget):
    ...
    def aug_bbox(self):
        ...
        aug_img = self.DB.list_img_id_TC("3", "1")

        if aug_img == None:
            aug_img = []
        else:
            aug_img = list(aug_img)
        if len(aug_img) != 0:
            aug_imgs.append(aug_img)
        aug_imgs = sum(aug_imgs, [])
        btn_names = aug_imgs
        for i in btn_names:
        ...
        if len(aug_imgs) >= 1:
            imgd = self.DB.get_table(str(current_object), "Image")[2]
    ...
    def save_info(self):
        ...
        if scale_factor_w > 1:
            if len(coordinates) != 0:
                for j, bbox in enumerate(bbox_info):
                    box_info = self.coordinate2bbox(coordinates[j])
                    self.DB.update_bbox(bbox[2], x=str(box_info[0] * scale_factor_w), y=str(box_info[1] * scale_factor_w), width=str(box_info[2] * scale_factor_w), height=str(box_info[3] * scale_factor_w))
                for i in range(len(self.a)):
            ...
            if len(coordinates) != 0:
                for j, bbox in enumerate(bbox_info):
                    box_info = self.coordinate2bbox(coordinates[j])
                    self.DB.update_bbox(bbox[2], x=str(box_info[0]), y=str(box_info[1]), width=str(box_info[2]), height=str(box_info[3]))
                for i in range(len(self.a)):
                    if self.a[i].isChecked():
                        self.b[i].setCheckState(Qt.Checked)
        ...
        #                     num = self.DB.get_obj_max_aug()
        #
        #                     self.DB.set_object(img_id, loc_id, str(j[4]), iteration, "-1", str(int(num + 1)))
    ...
    def bbox2cate_id(self, bbox):
        cate_table = self.DB.get_table(str(self.DB.get_table(str(bbox[1]), "Object")[2]), "Category")[0]
        return cate_table
